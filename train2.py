...
try:
    ...
    print("Warning: lpips library not found. Running without perceptual loss.")

# --- NEW: Total Variation (TV) Loss Function ---
def tv_loss(img):
    """Calculates Total Variation loss to smooth out GAN artifacts."""
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    lpips_fn = None
    if LPIPS_AVAILABLE:
        print("Loading VGG-based LPIPS model...")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if viewpoint_cam.alpha_mask is not None:
            image_hr *= viewpoint_cam.alpha_mask.cuda()

        # --- 2. Base HR Loss (L1 + SSIM) ---
        ...
        # --- 3. Perceptual Loss (LPIPS) to handle SwinIR textures ---
        loss_lpips = 0.0
        # Start LPIPS only after geometry settles so it doesn't cause chaotic floaters early on
        if LPIPS_AVAILABLE and lpips_fn is not None and iteration > 3000:
            image_lpips = (image_hr.unsqueeze(0) * 2.0) - 1.0
            gt_image_lpips = (gt_image_hr.unsqueeze(0) * 2.0) - 1.0
            loss_lpips = lpips_fn(image_lpips, gt_image_lpips).squeeze()

        # --- 4. Total Variation (TV) Regularization ---
        # Smooths out the micro-noise created by SwinIR
        loss_tv = tv_loss(image_hr)

        # --- 5. Anisotropy Penalty ---
        # Stops Gaussians from stretching into long needles
        scales = gaussians.get_scaling
        ratio = torch.max(scales, dim=1).values / (torch.min(scales, dim=1).values + 1e-7)
        loss_aniso = torch.where(ratio > 20.0, ratio, torch.zeros_like(ratio)).mean()

        # --- 6. Final Weighted Loss ---
        loss = loss_hr + (0.05 * loss_lpips) + (0.01 * loss_tv) + (0.05 * loss_aniso)

        # Depth regularization
        Ll1depth_pure = 0.0
        if depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:
            invDepth = render_pkg_std["depth"]
